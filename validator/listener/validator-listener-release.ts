...
const listenForReleaseEvents = async () => {
  const sourceChainId = 4;
  const targetChainId = 3;
  sourceChainProvider.on(sourceBridgeContract.filters.Release(), (event) =>
    handleReleaseEvent(event, sourceChainId, targetChainId)
  );
  targetChainProvider.on(targetBridgeContract.filters.Release(), (event) =>
    handleReleaseEvent(event, targetChainId, sourceChainId)
  );
};

export default listenForReleaseEvents;
